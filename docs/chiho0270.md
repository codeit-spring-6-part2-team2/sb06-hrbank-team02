...
> 요구사항을 다음과 같이 세부적으로 정의
  - 데이터 백업
     ...
     **Step2. 백업 데이터 생성**
      > i. 상태(`진행중`) 이력 등록   
      ii. 이전 판단 시점과 현재 시점의 변경 이력 생성시간 비교 (다를 경우 `실패` 처리 / 실패는 예외에서도 발생)  
...
처음부터 많은 내용을 공유하고 소통하다보니 되려 개발 시간 지연으로 이어져 발목을 잡혀버린 것이 아쉽습니다.

필요한 기능에 대한 요구사항을 해석하고 서로 맞추는 점이 매력적이었습니다.  
협업으로 진행하니 생각지 않은 부분까지 세세하게 알아갈 수 있었습니다.   
